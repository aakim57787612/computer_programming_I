...
void Polynomial::addition( Polynomial adder )
{
   ...
   for (int i = 0; i < size; i++) {
	   for (int j = 0; j < adder.size; j++) {
		   if (adder.terms[j].expon == terms[i].expon) {
			   k--;
		   }
	   }
   }
   ...
   for (int a = 0; a < k; a++) {
	   if (i == size) {
		   ...
		   sum.terms[a].expon = terms[i].expon;
		   sum.terms[a].coef = terms[i].coef;
		   i++;
	   }
	   else if (terms[i].expon == adder.terms[j].expon) {
			sum.terms[a].expon = terms[i].expon;
			sum.terms[a].coef = terms[i].coef + adder.terms[j].coef;
   ...
   if( size != 0 && multiplier.size != 0 )
   {
	   for (int j = 0; j < size; j++) {
		   for (int i = 0; i < multiplier.size; i++) {
			   buffer.terms[i].expon = terms[j].expon + multiplier.terms[i].expon;
			   buffer.terms[i].coef = terms[j].coef * multiplier.terms[i].coef;
		   }
