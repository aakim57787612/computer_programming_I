...
void addition( Polynomial &addend, const Polynomial adder )
{
   ...
   k = addend.size + adder.size;
   for (int i = 0; i < addend.size; i++) {
	   for (int j = 0; j < adder.size; j++) {
		   if (adder.terms[j].expon == addend.terms[i].expon) {
			   k--;
		   }
	   }
   }
   ...
   for (int a = 0; a < k; a++) {
	   if (i == addend.size) {
		   sum.terms[a].expon = adder.terms[j].expon;
		   sum.terms[a].coef = adder.terms[j].coef;
		   j++;
	   }
	   else if (j == adder.size) {
		   sum.terms[a].expon = addend.terms[i].expon;
		   sum.terms[a].coef = addend.terms[i].coef;
		   i++;
	   }
	   else if (addend.terms[i].expon == adder.terms[j].expon) {
		   sum.terms[a].expon = addend.terms[i].expon;
		   sum.terms[a].coef = addend.terms[i].coef + adder.terms[j].coef;
   ...
   if( multiplicand.size != 0 && multiplier.size != 0 )
   {
	   for (int j = 0; j < multiplicand.size; j++) {
		   for (int i = 0; i < multiplier.size; i++) {
			   buffer.terms[i].expon = multiplicand.terms[j].expon + multiplier.terms[i].expon;
			   buffer.terms[i].coef = multiplicand.terms[j].coef * multiplier.terms[i].coef;
		   }
		   addition(product, buffer);
	   }
	   while (product.size > 0 && product.terms[product.size - 1].coef == 0)
		   product.size--;
   }
   delete[] buffer.terms;
