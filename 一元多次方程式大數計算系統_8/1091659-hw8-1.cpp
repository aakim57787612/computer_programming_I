...
struct Term
{
   int coef;  // the coefficient of a term
   int expon; // the exponent of a term
};
...
void addition( Term *&addend, int &addendSize, Term *adder, int adderSize )
{
   ...
   k = addendSize + adderSize;
   for (int i = 0; i < addendSize; i++) {
	   for (int j = 0; j < adderSize; j++) {
		   if (adder[j].expon == addend[i].expon) {
			   k--;//­«½ÆExpon¼Æ¶q
		   }
	   }
   }
   ...
   for (int a = 0; a < k; a++) {
	   if (i == addendSize) {
		   sum[a].expon = adder[j].expon;
		   sum[a].coef = adder[j].coef;
		   j++;
	   }
	   else if (j == adderSize) {
		   sum[a].expon = addend[i].expon;
		   sum[a].coef = addend[i].coef;
		   i++;
	   }
	   else if (addend[i].expon == adder[j].expon) {
		   sum[a].expon = addend[i].expon;
		   sum[a].coef = addend[i].coef + adder[j].coef;
   ...
   Term *multiplier, int multiplierSize, Term *&product, int &productSize )
{
   // product = 0;
   productSize = 0;
   product = new Term[multiplicandSize + multiplierSize]();
   int bufferSize = multiplierSize;
   Term *buffer = new Term[bufferSize]();
   ...
   if( multiplicandSize != 0 && multiplierSize != 0 )
   {
	   for (int j = 0; j < multiplicandSize; j++) {
		   for (int i = 0; i < multiplierSize; i++) {
			   buffer[i].expon = multiplicand[j].expon + multiplier[i].expon;
			   buffer[i].coef = multiplicand[j].coef * multiplier[i].coef;
		   }
		   addition(product, productSize,buffer, bufferSize);
	   }
	   while (productSize > 0 && product[productSize - 1].coef == 0)
		   productSize--;
   }
   delete[] buffer;
